...
int getInd(char val) //ищем индекс элемента множества
{
    int ret=-1;
    int i;
...
void sumg(char a, char b, char save) // операция нахождения суммы 
{
    ...
    if(num_elems[ind_a]>num_elems[ind_b])
    {
        ...
        elems[ind_c]=(int *) calloc(num_elems[ind_c],sizeof(int)); //захватываем память для массива элементов
    ...
    else
    {
        ...
        elems[ind_c]=(int *) calloc(num_elems[ind_c],sizeof(int)); // то же самое, только другой массив
        for(i=0;i<num_elems[ind_b];i++)
        {
